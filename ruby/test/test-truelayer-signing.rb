...
class TrueLayerSigningTest < Minitest::Test
  ...
  def test_set_jku_should_succeed
    tl_signature = TrueLayerSigning.sign_with_pem
      ...
      .set_body({ event_type: "example", event_id: "18b2842b-a57b-4887-a0a6-d3c7c36f1020" }.to_json)
      .sign

    jws_header = TrueLayerSigning.extract_jws_header(tl_signature)

    assert_equal("https://webhooks.truelayer.com/.well-known/jwks", jws_header.jku)
  end

  def test_extract_jws_header_should_succeed
    tl_signature = File.read(File.expand_path("../../test-resources/webhook-signature.txt",
                                              File.dirname(__FILE__)))
    jws_header = TrueLayerSigning.extract_jws_header(tl_signature)
  ...
  # TODO: re-enable this test with the JWKS import is fixed
  # def test_verify_with_jwks_should_succeed
  #   tl_signature = File.read(File.expand_path("../../test-resources/webhook-signature.txt",
  #                                             File.dirname(__FILE__)))
  #   jwks = File.read(File.expand_path("../../test-resources/jwks.json", File.dirname(__FILE__)))

  #   result = TrueLayerSigning.verify_with_jwks(jwks)
  #     .set_method(:post)
  #     .set_path("/tl-webhook")
  #     .add_header("x-tl-webhook-timestamp", "2021-11-29T11:42:55Z")
  #     .add_header("content-type", "application/json")
  ...
  def test_verify_with_jwks_with_header_mismatch_should_fail
    tl_signature = File.read(File.expand_path("../../test-resources/webhook-signature.txt",
                                              File.dirname(__FILE__)))
    jwks = File.read(File.expand_path("../../test-resources/jwks.json", File.dirname(__FILE__)))
    ...
    verifier = TrueLayerSigning.verify_with_jwks(jwks)
      .set_method(:post)
      .set_path("/tl-webhook")
      .add_header("x-tl-webhook-timestamp", "2021-12-02T14:18:00Z")
      .add_header("content-type", "application/json")
      .set_body({ event_type: "example", event_id: "18b2842b-a57b-4887-a0a6-d3c7c36f1020" }.to_json)
